...
select_parts_and_measures():
    ...
    Displays a selection menu to the user with several options related to the selection of musical parts and measures.

select_measure_range(score):
    Prompts the user to input a valid measure range in a given musical score.
    Displays a menu to guide the user through the process of selecting a range of measures in the given musical score.
...
>>> from music21 import *
>>> selected_part = select_parts_and_measures()
...
from routines_name_parts import selected_score_part_names
...
def select_parts_and_measures() -> music21.stream.Score or music21.stream.Part:
    """
    Facilitates the user's selection of musical parts and measures from a global score.
    ...
    - "SLCT: Full Score - Range of Measures": Select the full score and a range of measures.
    - "PART: Part - All Measures": Select an individual part and all measures.
    - "SLCT: Part - Range of Measures": Select an individual part and a range of measures.
    - "PRVS: Previous Score Selection": Repeat the previous score selection.
    ...
    Returns
    -------
    music21.stream.Score or music21.stream.Part
        The selected part of the score based on the user's choice. The part contains either all measures
        or a specific range of measures, depending on the user's choice.
    ...
    if choice == "prev_selection":  # Repeat previous selection
        if previous_score_selection:
            return previous_score_selection[0]
        else:
    ...
    if choice == "full_all":  # Full score with all measures
        previous_score_selection = (selected_score_part_names['full'], (
            1, len(selected_score_part_names['full'].parts[0].getElementsByClass(stream.Measure))))
        return selected_score_part_names['full']

    if choice == "full_range":  # Full score with a range of measures
        range_ = select_measure_range(selected_score_part_names['full'])
        score_selected_measures = selected_score_part_names['full'].measures(*range_)
        previous_score_selection = (score_selected_measures, range_)
        return score_selected_measures
...
# score = corpus.parse('bwv66.6')
#
# selected_score_part_names = {'full': score}
#
