...
def example_submenu_entries_dict() -> dict:
    """
    Generate a representative dictionary for submenu entries.

    This function creates and returns a dictionary that simulates a submenu. This dictionary is structured to be used
    by the `display_menu_undirected()` function. The dictionary includes the text displayed for the submenu and a
    list of submenu entries.

    The "menu_displayed_text" field in the dictionary includes a title for the submenu, an instruction for selection,
    a representative entry layout, and a prompt for the user's choice.

    The "menu_entries" field is a list of potential options available in the submenu. Each entry is a list itself,
    with the first element being a brief description of the entry,the second element being the function that will be
    executed when this entry is selected, and the third element being a more detailed explanation of what the entry
    does.
    ...
    1. **If another submenu is selected** (e.g., "ABBR: Option 1 titel"):
       The corresponding function (e.g., `example_sub_submenu_entries_dict()`) is expected to return yet anothe
       dictionary that represents the sub-submenu. The function will then recursively call itself with
    ...
    3. **If the 'back' option is selected**:
       The `display_menu_undirected()` function will return, reverting to the previous submenu or
       the main menu if it was a top-level submenu.

    4. **If the 'main-menu' option is selected**:
       The `display_menu_undirected()` function will return all the way up to the main menu.

    After the function or navigation option is executed, the user will return to the corresponding menu.
    ...
    ...    ],
    ...    "menu_entries": [
    ...        ["ABBR: Sub-Option 1 title", example_sub_submenu_entries_dict, "<Goes to submenu>"],
    ...        ["ABBR: Sub-Option 2 title", example_some_function, "<Executes demo function>"],
    ...
    return {
        ...
        "menu_entries": [
            ["ABBR: Sub-Option 1 titel", example_sub_submenu_entries_dict, "<Goes to sub-submenu>"],
...
def example_unidirectional_menu_entries_dict() -> dict:
    """
    Generate a representative dictionary for unidirectional menu entries.

    This function creates and returns a dictionary that simulates a unidirectional menu. This dictionary is
    structured to be used by the `display_menu_directed()` function. The dictionary includes the text displayed for
    the menu and a list of menu entries.
    ...
    When the `display_menu_directed()` function uses this dictionary, it behaves as follows:

    1. **If a submenu is selected** (e.g., "ABBR: Option 1 title"):
       The corresponding function (e.g., `example_submenu_entries_dict()`) is expected to return another dictionary
       that represents the submenu. The function will then recursively call itself with the new dictionary,
       thus entering the submenu.

    2. **If a regular function is selected** (e.g., "ABBR: Option 2 title"):
       The corresponding function (e.g., `example_some_function()`) is executed.

    In both cases, after the function is executed, the user will return to the menu where the function was selected.
    ...
    See Also
    --------
    display_menu_directed : function that displays and navigates unidirectional menus
...
def demo_menu_structure_dict() -> dict:
    ...
    return {
        ...
        "menu_entries": [
            ...
            ["DEMO: Display demo results", (generic_analysis_workflow_single, example_analysis), "<Starts an example analysis workflow>"],
            ["BACK: Return to the last menu", 'back', "<Returns to the parent menu>"],
            ["MAIN: Return to the main menu", 'main-menu', "<Returns to main menu>"],
        ]
    }
...
def demo_function_directed_menu() -> None:
    """
    A demo function that demonstrates menu navigation.

    This function demonstrates the behaviour of a menu after executing a function in the directed menu. Specifically,
    it shows that after a function is executed, the directed menu instance will be ended. This function could also be
    used within an analysis function to get input from the user and direct the flow of analysis based on the user's
    selection.

    This function does not take any arguments or return any value. Instead, it simply executes the
    `display_menu_directed` function with the `example_mainmenu_entries_dict` function as an argument.

    See Also
    --------
    display_menu_directed : Function that displays a menu to the user, allows user selection, and calls the
                            appropriate function.
    example_mainmenu_entries_dict : A function that returns a dictionary representing a main menu.
    """
